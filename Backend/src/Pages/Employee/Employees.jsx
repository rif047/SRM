...
export default function Employee() {
    ...
    const handleDelete = async (id) => {
        ...
        if (shouldDelete) {
            try {
                await axios.delete(`${process.env.REACT_APP_API_URL}/api/employees/delete/${id}`);
                fetchData();
    ...
    const handleSearch = async (term) => {
        ...
        const filtered = employees.filter((employee) => {
            let nameMatch = employee.name.toLowerCase().includes(term.toLowerCase());
            let phoneMatch = employee.phone.includes(term);
            let work_idMatch = employee.work_id.includes(term);
            let nidMatch = employee.nid.includes(term);
            let altPhoneMatch = employee.alt_phone && employee.alt_phone.includes(term);
            let addressMatch = employee.address.toLowerCase().includes(term.toLowerCase());
            let emailMatch = employee.email && employee.email.toLowerCase().includes(term.toLowerCase());

            return nameMatch || phoneMatch || altPhoneMatch || addressMatch || emailMatch || work_idMatch || nidMatch;
        });
    ...
    return (
        <Layout>
            ...
            <div className="flex justify-end mb-5">
                <NavLink to={'/employees/create'} className="flex justify-center bg-[#4c5165] text-gray-300 font-bold px-5 rounded-md">
                    New
                    <AddOutlinedIcon />
                </NavLink>
            </div>
            ...
            <table className="w-full text-xs text-left text-gray-500 shadow-md rounded-lg mt-2 mb-2" id="table-to-xls">
                ...
                <tbody className="[&>tr>td]:px-4 [&>tr>td]:py-1 [&>tr>td]:font-normal [&>tr>td]:text-gray-900 [&>tr>td]:text-center [&>tr>td]:border">
                    {displayedEmployees.map((employee, index) => (
                        <tr key={employee._id} className="bg-white border-b hover:bg-gray-100">
                            <td className=" w-[80px]">
                            ...
                            <td>
                                {employee.alt_phone === undefined || employee.alt_phone === employee.phone ? 'N/A' : employee.alt_phone}
                            </td>
                            <td>
                                {employee.address}
                            </td>
                            ...
                            <td>
                                {employee.email === undefined || employee.email === 'rif.srm@gmail.com' ? 'N/A' : employee.email}
                            </td>
                            <td className=" w-[100px]">

                                <NavLink
                                    to={`/employees/update/${employee.phone}`}
                                    className="mr-3 [&>svg]:text-yellow-400"
                                >
                                    <EditOutlinedIcon />
                                </NavLink>

                                <button onClick={() => handleDelete(employee._id)} className="[&>svg]:text-red-400"><DeleteOutlineOutlinedIcon /></button>
                            </td>
                        </tr>
                    ))}
                </tbody>
